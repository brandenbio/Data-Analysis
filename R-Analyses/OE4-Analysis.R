# --------------------------------------------------------------------------------------------------
# Omniscience Errors Experiment 4 Analysis
# --------------------------------------------------------------------------------------------------
# Subject pool: Amazon Mechanical Turk
# Date(s):      November 2023
...
# 1. Omnibus table
# --------------------------------------------------------------------------------------------------
omnibus.table <- group_by(data, ProblemType, Construction, ExplicitDenial) %>% 
...
# ProblemType Construction  ExplicitDenial     n   mean    sd     se     ic
# 1 nonFactive  Modus Ponens  noDenial          62 0.323 0.471 0.0599 0.120
# 2 Factive     Modus Ponens  noDenial          62 0.306 0.465 0.0590 0.118
# 3 Factive     Modus Ponens  Denial            62 0.290 0.458 0.0581 0.116
# 4 Factive     Modus Tollens Denial            62 0.290 0.458 0.0581 0.116
# 5 nonFactive  Modus Tollens noDenial          62 0.290 0.458 0.0581 0.116
# 6 nonFactive  Modus Tollens Denial            62 0.274 0.450 0.0571 0.114
# 7 Factive     Modus Tollens noDenial          62 0.242 0.432 0.0548 0.110
# 8 nonFactive  Modus Ponens  Denial            62 0.242 0.432 0.0548 0.110
...
# Cliff's Delta
# 
# delta estimate: 0.01456816 (negligible)
# 95 percent confidence interval:
#   lower      upper 
# -0.2306573  0.2580536 

# 2.3) Wilcoxon test comparing errors of omniscience - ExplicitDenial
data %>% group_by(ExplicitDenial) %>% summarise(OmniscienceError = mean(OmniscienceError))
# ExplicitDenial    OmniscienceError
# 1 Denial                  0.177
# 2 noDenial                0.153

Denial.OE <- aggregate(OmniscienceError ~ ParticipantID, data=data[data$ExplicitDenial=="Denial",], FUN=mean)
noDenial.OE <- aggregate(OmniscienceError ~ ParticipantID, data=data[data$ExplicitDenial=="noDenial",], FUN=mean)

wilcoxsign_test(Denial.OE$OmniscienceError ~ noDenial.OE$OmniscienceError)
# Asymptotic Wilcoxon-Pratt Signed-Rank Test
# data:  y by x (pos, neg) 
# stratified by block
# Z = 1.1486, p-value = 0.2507
# alternative hypothesis: true mu is not equal to 0

cliff.delta(Denial.OE$OmniscienceError,noDenial.OE$OmniscienceError,conf.level = 0.95)
# Cliff's Delta
# 
# delta estimate: -1.066715e-17 (negligible)
# 95 percent confidence interval:
#   lower      upper 
# -0.2556856  0.2556856 
...
# 2.5) Wilcoxon test on errors of omniscience for interaction between Construction and Denial
data %>% group_by(Construction, ExplicitDenial) %>% summarise(OmniscienceError = mean(OmniscienceError))
# Construction    ExplicitDenial OmniscienceError
# 1 Modus Ponens  Denial                  0.5  
# 2 Modus Ponens  noDenial                0.401
# 3 Modus Tollens Denial                  0.378
# 4 Modus Tollens noDenial                0.285

ConstructionxExplicitDenial.OE <- data %>% group_by(ParticipantID, Construction, ExplicitDenial) %>% 
  summarise(OmniscienceError = mean(OmniscienceError))

ConstructionxDenial.OE <- subset(ConstructionxExplicitDenial.OE, Construction=="Modus Ponens" & ExplicitDenial=="Denial")$OmniscienceError - 
  subset(ConstructionxExplicitDenial.OE, Construction=="Modus Tollens" & ExplicitDenial=="Denial")$OmniscienceError
ConstructionxnoDenial.OE <- subset(ConstructionxExplicitDenial.OE, Construction=="Modus Ponens" & ExplicitDenial=="noDenial")$OmniscienceError - 
  subset(ConstructionxExplicitDenial.OE, Construction=="Modus Tollens" & ExplicitDenial=="noDenial")$OmniscienceError

wilcoxsign_test(ConstructionxDenial.OE ~ ConstructionxnoDenial.OE)
# Asymptotic Wilcoxon-Pratt Signed-Rank Test
# data:  y by x (pos, neg) 
# stratified by block
...
cliff.delta(ConstructionxDenial.OE, ConstructionxnoDenial.OE, conf.level = 0.95)
# Cliff's Delta
# 
...
# 2.6) Wilcoxon test on errors of omniscience for interaction between ProblemType and Denial
data %>% group_by(ProblemType, ExplicitDenial) %>% summarise(OmniscienceError = mean(OmniscienceError))
#   ProblemType   ExplicitDenial OmniscienceError
# 1 Factive     Denial                  0.390
# 2 Factive     noDenial                0.267
# 3 nonFactive  Denial                  0.488
# 4 nonFactive  noDenial                0.419

VerbxExplicitDenial.OE <- data %>% group_by(ParticipantID, ProblemType, ExplicitDenial) %>% 
  summarise(OmniscienceError = mean(OmniscienceError))

VerbxDenial.OE <- subset(VerbxExplicitDenial.OE, ProblemType=="nonFactive" & ExplicitDenial=="Denial")$OmniscienceError - 
  subset(VerbxExplicitDenial.OE, ProblemType=="Factive" & ExplicitDenial=="Denial")$OmniscienceError
VerbxnoDenial.OE <- subset(VerbxExplicitDenial.OE, ProblemType=="nonFactive" & ExplicitDenial=="noDenial")$OmniscienceError - 
  subset(VerbxExplicitDenial.OE, ProblemType=="Factive" & ExplicitDenial=="noDenial")$OmniscienceError

wilcoxsign_test(VerbxDenial.OE ~ VerbxnoDenial.OE)
# Asymptotic Wilcoxon-Pratt Signed-Rank Test
# data:  y by x (pos, neg) 
# stratified by block
...
cliff.delta(VerbxDenial.OE, VerbxnoDenial.OE, conf.level = 0.95)
# Cliff's Delta
# 
...
# 2.7) Wilcoxon test on errors of omniscience for three-way interaction
data %>% group_by(ProblemType, Construction, ExplicitDenial) %>% summarise(OmniscienceError = mean(OmniscienceError))
#   ProblemType   Construction  ExplicitDenial OmniscienceError
# 1 Factive     Modus Ponens  Denial                  0.407
# 2 Factive     Modus Ponens  noDenial                0.337
# 3 Factive     Modus Tollens Denial                  0.372
# 4 Factive     Modus Tollens noDenial                0.198
# 5 nonFactive  Modus Ponens  Denial                  0.593
# 6 nonFactive  Modus Ponens  noDenial                0.465
# 7 nonFactive  Modus Tollens Denial                  0.384
# 8 nonFactive  Modus Tollens noDenial                0.372

omni.test <- data %>% group_by(ParticipantID, ProblemType, Construction, ExplicitDenial) %>% 
                            summarise(OmniscienceError = mean(OmniscienceError))

omni.A1 <- subset(omni.test, ProblemType=="nonFactive" & Construction=="Modus Ponens" & ExplicitDenial=="Denial")$OmniscienceError -
  subset(omni.test, ProblemType=="nonFactive" & Construction=="Modus Tollens" & ExplicitDenial=="Denial")$OmniscienceError
omni.A2 <- subset(omni.test, ProblemType=="nonFactive" & Construction=="Modus Ponens" & ExplicitDenial=="noDenial")$OmniscienceError -
  subset(omni.test, ProblemType=="nonFactive" & Construction=="Modus Tollens" & ExplicitDenial=="noDenial")$OmniscienceError

omni.B1 <- subset(omni.test, ProblemType=="Factive" & Construction=="Modus Ponens" & ExplicitDenial=="Denial")$OmniscienceError -
  subset(omni.test, ProblemType=="Factive" & Construction=="Modus Tollens" & ExplicitDenial=="Denial")$OmniscienceError
omni.B2 <- subset(omni.test, ProblemType=="Factive" & Construction=="Modus Ponens" & ExplicitDenial=="noDenial")$OmniscienceError -
  subset(omni.test, ProblemType=="Factive" & Construction=="Modus Tollens" & ExplicitDenial=="noDenial")$OmniscienceError
...
# For access
wilcoxsign_test(Denial.OE$OmniscienceError ~ zeroes.OE$OmniscienceError)
# Asymptotic Wilcoxon-Pratt Signed-Rank Test
# data:  y by x (pos, neg) 
# stratified by block
...
cliff.delta(Denial.OE$OmniscienceError, zeroes.OE$OmniscienceError, conf.level = 0.95)
# Cliff's Delta
# 
...
# For no access
wilcoxsign_test(noDenial.OE$OmniscienceError ~ zeroes.OE$OmniscienceError)
# Asymptotic Wilcoxon-Pratt Signed-Rank Test
# data:  y by x (pos, neg) 
# stratified by block
...
cliff.delta(noDenial.OE$OmniscienceError, zeroes.OE$OmniscienceError, conf.level = 0.95)
# Cliff's Delta
#
...
# 3.2)  Omniscience Errors for Denial access
fig.Denial.df <- group_by(data, ExplicitDenial) %>% 
  summarise( 
    n=n(),
    mean=mean(OmniscienceError),
    sd=sd(OmniscienceError)
...
fig.Denial.df.temp <- fig.Denial.df[,3:6]*100

fig.Denial.df <- cbind(fig.Denial.df[1:2], fig.Denial.df.temp)

Denial.bPplot <- ggplot(fig.Denial.df, aes(y=mean, x=ExplicitDenial)) +
  geom_bar(stat="identity", position=position_dodge(), fill="brown3", size=.5, width=.75) +
  geom_errorbar(aes(x=ExplicitDenial, ymin=mean-ic, ymax=mean+ic), 
                position=position_dodge(width=0.9), width=0.2, alpha=0.9, size=.5) +
  scale_y_continuous(name = "Omniscience Errors", limits = c(0, 105), breaks = c(0, 50, 100)) +
  scale_x_discrete(name = "", labels=c("Denial", "No Denial")) +
  theme_minimal() + 
  theme(legend.title = element_blank(),
        legend.position=c(0.26, 0.72),
        legend.key.size = unit(.5, 'cm'),
...
# 3.4)  Violin plot of omniscience errors for interaction between ProblemType and Denial
fig.interaction.df <- aggregate(OmniscienceError ~ ParticipantID * ExplicitDenial * Verb, data=data,
                  FUN=function(x){ round(mean(x), digits=2) })

interaction.vPlot <- ggplot(fig.interaction.df, aes(x=ExplicitDenial, y=OmniscienceError)) +
