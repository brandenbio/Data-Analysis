# --------------------------------------------------------------------------------------------------
# Relational Tracking Experiment 1 - Modeling and Simulation Analysis
# --------------------------------------------------------------------------------------------------
...
project <- "RT1"
data <- read.csv(paste("/Users/bbio/Repository/Projects/Perception/Relational Tracking/modeling/", project,"/", project, "-humanData.csv", sep=""), header=T)
  ...
  for (co in celloffset) {
    for (d in distances) {
      for (com in compression) {
        current_file_pattern <- paste("cs", as.character(cs), "_co", as.character(co), "_d", as.character(d), "_com", as.character(com), ".csv", sep="")
        ...
        #grid.arrange(ambiguous.hum.heatmap, ambiguous.sim.heatmap, ncol = 2, top = textGrob(paste("Heatmap_", current_file_pattern, sep=""),gp=gpar(fontsize=20,font=3)))
        ...
        #grid.arrange(unambiguous.hum.heatmap, unambiguous.sim.heatmap, ncol = 2, top = textGrob(paste("Heatmap_", current_file_pattern, sep=""),gp=gpar(fontsize=20,font=3)))
